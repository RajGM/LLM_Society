% Draft reproducibility appendix for the full thesis.
% Expected parent packages: booktabs, longtable, array, listings, xcolor,
% hyperref, xurl, amsmath, and enumitem.
...
Paths are relative to the repository root.  The canonical machine-readable
design is \path{thesisExperiment/configs/grid_phase2.json}; where this appendix
and a generated artefact disagree, that file and the raw run metadata take
precedence.  The campaign used one graph seed (\texttt{graphRandomSeed: 42})
and one replicate, and therefore is not a replicated inferential experiment.
Continuous and dual-discrete misinformation indices are separate instruments
...
and auditor, eight nodes, eight ticks, a maximum of eight hops, one drip seed
(\texttt{node\_0}), an inbox cap of four, an always-active schedule, relation
evolution with \texttt{trustDelta=0.05}, and six seed articles.  Graph
configurations use action weights 0.35/0.50/0.15 for
forward/reinterpret/drop and trust threshold 0.15.  The linear chain is the
deliberately rewrite-heavy exception: 0.10/0.85/0.05 and threshold 0.08.
...
$v_7\rightarrow v_0$.\\
\texttt{random\_er} & $p=0.42$; minimum seed out-degree 2 & Each ordered
non-self pair is sampled; a guard adds outgoing seed edges if required.\\
...
stored in \path{thesisExperiment/articles/merged.json}.  The inventories above
identify experimental inputs; they do not reproduce the full stimuli.
...
\texttt{dual\_gap.csv} is narrow:
\texttt{experimentName, articleId, topology, meanDualGap, meanAgreement,
meanDiscreteMI, meanContinuousMI}; join it to the wide dual table on
\texttt{(experimentName, articleId)}.  Pair T2c with T2d on
\texttt{(topology, rest, articleId)}, never by row number.  Dead cells remain
in the ledger and are excluded from live means rather than imputed as zero.
...
The present file is an input fragment.  Once the full thesis master exists,
include it with \verb|\input{drafts/15_APPENDICES}| and compile that master
with its normal \texttt{latexmk} or \texttt{pdflatex}/\texttt{biber} sequence.
No master file is assumed in this draft.
...
\section{Credential handling and consolidation provenance}
\label{app:security-provenance}

The OpenAI credential is runtime infrastructure, not a research artefact.
It must be supplied through \texttt{OPENAI\_API\_KEY} or an ignored local
\path{.env} file.  Never paste a value into a configuration, manuscript,
terminal transcript, issue, manifest, or committed file; never print
\path{.env}.  The safe readiness check is
\begin{lstlisting}[language=bash,basicstyle=\ttfamily\small]
node thesisExperiment/scripts/check_openai_key.js
\end{lstlisting}
which reports only presence, length, and candidate locations.  Repository
ignore rules cover \path{.env}, \path{.env.local}, and
\path{.env.*.local}.  Raw metadata records usage and model names but no API
credential.
...
canonical parse ledger contains 292 dead T2 cells; older 290-cell analysis
copies remain under
\path{thesisExperiment/CONSOLIDATED_OVERLAPS/analysis_phase2_hatch290/} for
auditability and must not replace the canonical tables.
