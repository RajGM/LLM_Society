...
The explanation has three levels.
Persona composition defines the agents that transform a message.
Topology defines who can expose whom.
Propagation joins these elements over time.
...
\emph{Operationalization} states how the relation is represented in the simulation.
...
\subsection{The integrated causal argument}
...
Yet the controlled-random heterogeneous branches reached the propaganda tier in about \(85\%\) of branch--domain pairs.
...
\paragraph{Evidence.}
The award paper provides simulation evidence for persona-conditioned drift on linear branches.
...
\paragraph{Theory.}
Topology does not directly alter a sentence.
It changes the opportunities for alteration.
...
The design includes chains, rings, random graphs, small-world graphs, scale-free graphs, echo chambers, polarized graphs, and hierarchical graphs.
...
The linear chain is the direct link to the award paper.
The small-world graph is the clearest named operationalization of local clustering \parencite{watts1998collective}.
...
One graph seed per cell does not identify a topology-wide effect.
...
In the award paper, the answer vector has ten binary items.
...
Its numerical range therefore differs from the award paper's ten-item instrument.
...
The propaganda threshold is also operational.
The award paper labels averages at or below \(1\) as factual error.
It labels values above \(1\) and at or below \(3\) as lies.
It labels values above \(3\) as propaganda \parencite{maurya2025simulating}.
...
The award paper assumes a perfect auditor in its limitations.
...
\paragraph{Theory.}
The award paper studies sequential transmission chains.
...
They cannot reproduce the award paper merely by reusing MI and MPR.
...
Identity composition operationalizes part of lack of diversity.
Graph generators operationalize selected cluster and flow conditions.
Seed fan-out can operationalize shock versus drip.
Ticks and activity schedules can operationalize simulated opportunity rates.
...
They organize the empirical analysis.
...
\subsection{Theory, operationalization, and evidence ledger}
...
\toprule
Construct & Theory & Operationalization & Evidence boundary \\
\midrule
Persona composition
...
& Generator-specific evidence. One graph seed does not identify a topology class. \\
...
The thesis joins these elements in a complex-network design.
It then tests where the bridge holds, where it depends on measurement, and where it fails.
