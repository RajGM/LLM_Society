...
heterogeneous conditions of the CIKM/LASS simulation study
...
\end{equation}
Each configuration was executed once (\(N=1\)).  Accordingly, article cells
...
Articles were processed sequentially within a configuration.  Before each
article, its prior history was removed and all node inboxes were flushed.
...
The heterogeneous arm (\(He\)) uses six fixed, ordered mixes of eight
identifiers.  The mixes vary composition rather than drawing personas at
runtime.  In particular, \texttt{mix\_00} contains four conspiracy and four
...
sequential generators.  Echo-chamber and polarised heterogeneous
configurations instead request cluster-based assignment from declared persona
pools; unavailable identifiers are filtered by the engine before assignment.
Consequently, \(H\) versus \(He\) is a controlled composition contrast, not a
generic test of ``diversity'' in the abstract.
...
For generators that use persona homophily, the source code adjusts a base
trust according to the Jaccard overlap of persona tags and clips the result to
\([0.05,0.95]\).
...
\begin{table}[tbp]
  ...
  \begin{tabularx}{\textwidth}{@{}lX@{}}
    ...
    Random ER &
      ...
      \texttt{minSeedOutDegree}=2 is present in the configuration
...
CIKM/LASS design.  The other generators test cyclic, random, clustered,
hub-dominated, ideologically partitioned, and authority--audience structures.
These small graphs are experimental mechanisms; they are not claimed to be
fitted reconstructions of a social-media platform.

\subsection{Seeding and propagation protocol}
\label{subsec:simulation-propagation}

Each article is drip-seeded into \texttt{node\_0} only.  The seed message has
...
count has reached the configured limit or if source trust is below the node's
threshold.  It then samples one of three configured actions:
\[
  A(x)\in\{\text{forward},\text{reinterpret},\text{drop}\}.
\]
Linear-chain configurations use
\begin{equation}
...
the trust threshold.  The other main-grid configurations use
\begin{equation}
...
\begin{figure}[tbp]
  ...
  \begin{minipage}{0.94\textwidth}
    ...
    \end{enumerate}
  \end{minipage}
  \caption{Pseudocode for propagation and post-hoc auditing of one
  configuration.}
...
Both rewriting agents and the auditor use \texttt{gpt-4o-mini}.  The agent is
conditioned by the assigned persona's system prompt.  The auditor receives the
...
\subsection{Eight-hop horizon and computational budget}
\label{subsec:eight-hop-budget}

The CIKM/LASS chain used 30 positions or hops
\parencite{maurya2025simulating}.  Phase~2 limits each graph to eight nodes,
eight ticks, and a maximum of eight message hops.  This is an explicit
...
paid model calls.  The eight-hop horizon therefore prioritises factorial
coverage over long-tail cascade depth.
...
cascade has converged, and it weakens any claim of irreversible late-stage
...
hatched after a real LLM run; it is retained in the ledger, excluded from live
means, and never imputed as zero.
...
The Phase~2 experiment evaluated how climate and geoengineering messages changed when
persona-conditioned agents propagated them through a simulated society.  A ground-truth
article was introduced at a designated seed node, agents could forward, reinterpret, or
drop the message, and an auditor scored the resulting text against five article-specific
ground-truth questions.  Both the acting agents and the auditor used
\texttt{gpt-4o-mini}.  The resulting scores are therefore LLM-as-judge measurements,
not human annotations and not measurements observed on Twitter.

The main factorial grid crossed eight generated topologies with two identity
arrangements and two scoring configurations.  The topologies were linear chain, ring,
Erd\H{o}s--R\'enyi, small world, scale free, echo chamber, polarised, and hierarchical.
The homogeneous arm (H) placed one of 12 personas throughout an eight-node graph,
whereas the heterogeneous arm (He) used one of six eight-persona mixes.  Every
configuration processed the same six core articles.  Runs used eight ticks, eight hops,
one graph seed (\texttt{graphRandomSeed: 42}), and one replicate.  This produced
\(1{,}728\) article-level cells:
\[
  (8\times 12\times 2\times 6)
  +(8\times 6\times 2\times 6)=1{,}728.
\]
The harvested table contains all \(1{,}728\) expected cells and reports no missing cell.
The eight-tick budget was a computational cut relative to the 30-hop CIKM setup; it is
not Debnath et al.'s ``eight'', which refers to a word-embedding context window.
...
\subsection{Network threshold \(k^{*}\)}
\label{sec:mm-kstar}
...
irreversible crossing is observed, \(k^{*}\) is undefined rather than zero.  The
observation window ends at eight ticks, so a crossing at the final observed tick is
right-censored and provides only weak evidence of persistence.  T2d \(k^{*}\) is
...
T2c\_H, 98 in T2d\_H, 50 in T2c\_He, and 48 in T2d\_He.  The four live populations are
therefore 480, 478, 238, and 240 cells, respectively.  All reported means, medians,
paired-cell contrasts, and exploratory tests use the live subset unless a count is
...
The five comparison families were designed to make the held and varied dimensions
...
Three non-parametric procedures accompany the descriptive tables:
\begin{itemize}
  \item two-sided paired Wilcoxon signed-rank tests compare the eight aligned topology
        means in C1--C3 and, separately, the paired live cells in C2 and C3;
  \item Spearman correlation describes the rank agreement of topology means between
        instruments in C2 and C3; and
  \item two-sided Mann--Whitney \(U\) tests compare pooled live H and He cells,
        conspiracy and science/environment H cells, and \texttt{mix\_00} and
        \texttt{mix\_02} He cells in C5, always within one instrument.
\end{itemize}
These tests are exploratory.  The eight topology means are design strata rather than
independent campaign replications, and pooled cells share one graph seed and are
nested within topology, persona or mix, and article.  With \(N=1\), \(p\)-values do
not provide confirmatory replicate-level inference.  No causal claim, confidence
interval, or population-level firestorm effect is inferred from them.  The harvest's
\texttt{thesisGrade} flag is \texttt{false}.
...
\begin{equation}
  ...
  \label{eq:dtfs}
\end{equation}
with each component clipped to \([0,1]\).  The implementation's validation threshold
...
\begin{itemize}
  \item IFD formulas, dual calls, absolute gap, and agreement:
        \texttt{\detokenize{src/Auditor.js}};
  \item event collection, cell means, node means, \(k^{*}\), and hatch policy:
        \texttt{\detokenize{thesisExperiment/scripts/parse_phase2.js}};
  \item harvested counts and the \(N=1\)/non-thesis-grade status:
        \texttt{\detokenize{thesisExperiment/results_phase2/summary.json}};
  \item C1--C5 joins, pooled summaries, and exploratory procedures:
        \texttt{\detokenize{thesisExperiment/analysis_full/analyze_full.py}} and
        \texttt{\detokenize{thesisExperiment/analysis_full/tables/}};
  \item structural metric extraction:
        \texttt{\detokenize{src/ValidationMetrics.js}};
  \item KS, JS, structural similarity, and DTFS definitions:
        \texttt{\detokenize{src/ValidationComparison.js}}; and
  \item D-net validation inputs and outputs:
        \texttt{\detokenize{thesisExperiment/results_phase2/debnath_compare.json}}.
\end{itemize}
