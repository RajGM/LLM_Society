...
results. Dual-discrete scores exceeded continuous scores on every
network-condition aggregate in both arms, while topology ranks aligned
weakly. Same-event dual diagnostics show the same directional gap, but
agreement is defined for only 270 of 478 live H dual cells and 171 of 240
live He dual cells. Because missing items receive different penalties, a
common $\mathrm{MI}>3$ cut is not equivalent across instruments. T2c and
T2d are also separate realisations, so campaign-level T2d--T2c contrasts are
whole-pipeline sensitivity rather than a clean instrument effect. The
campaign therefore establishes that severity levels and network-condition
ordering depend on the auditor regime. It does not separately answer an
instrument effect on threshold occurrence from a dedicated topology-resolved
\(k^{*}\) table.
...
\paragraph{Evidence.}
Topology changed the live-cell scores, the frequency of within-window
...
($n=198$).  Instrument choice also affected the incidence of within-window
...
weakly across modes (Spearman $\rho=0.095$ for H and $\rho=0.167$ for He).
These coefficients describe this harvest's rank order; they are not
