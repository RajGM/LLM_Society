...
($N=1$), one retained realisation per configuration, an eight-tick horizon, and 292 dead cells among
...
\subsection{RQ1: Auditor-regime sensitivity of levels and network-condition orderings}

\paragraph{Evidence.}
Dual-discrete scores exceeded continuous scores on every
network-condition aggregate in both arms. The topology-mean difference was
$0.8201$ for homogeneous and $1.0450$ for heterogeneous runs. Among cells
live under both modes, the median paired difference was $0.8125$ for H
($n=398$) and $0.9778$ for He ($n=198$). Topology ranks aligned weakly
(Spearman $\rho=0.095$ for H and $\rho=0.167$ for He). These coefficients
describe this harvest's rank order; they are not population inferences.
Same-event dual diagnostics show the same directional gap, but agreement is
defined for only 270 of 478 live H dual cells and 171 of 240 live He dual
cells. Instrument choice also affected the incidence of within-window
\kstar{}: the collapsed He rates were $32.1\%$ under dual-discrete scoring
and $7.6\%$ under continuous scoring. Because missing items receive different
penalties, a common $\mathrm{MI}>3$ cut is not equivalent across instruments.
T2c and T2d are also separate realisations, so campaign-level T2d--T2c
contrasts are whole-pipeline sensitivity rather than a clean instrument
effect.

\paragraph{Interpretation.}
The instrument comparison constrains the interpretation of all substantive
results. The two modes are not interchangeable measurements on a common numerical
...
claims about network conditions. The continuous sidecar generated during a
dual run supports diagnosis of disagreement, but it is not a third
independent \MPR{}. The campaign therefore establishes that severity levels
and network-condition ordering depend on the auditor regime. It does not
separately answer an instrument effect on threshold occurrence from a
dedicated topology-resolved \(k^{*}\) table.
...
\subsection{RQ2: Named persona families and heterogeneous compositions}
...
Restricting the collapsed comparison to zero-HTTP-error runs reverses the
continuous difference ($-0.0295$ rather than $+0.1549$) while the
dual-discrete difference remains positive ($+0.4695$). The continuous
collapsed-arm direction is therefore not robust to API availability.
...
Consequently, the aggregate He$>$H difference is consistent with a strong
...
\subsection{RQ3: Bundled network conditions, live-cell mean MI, and cascade survival}

\paragraph{Evidence.}
The eight bundled network conditions differed in live-cell scores, the
frequency of within-window \kstar{}, and the probability that a cascade
remained scorable. No condition occupied the highest or lowest position
consistently across identity composition and scoring instrument.  In homogeneous continuous runs,
...
\subsection{RQ4: Constructed hashtag-graph stress test}
...
instruments. Continuous scores were $1.7349$ versus $0.9983$ for SCoPEx and
$3.3378$ versus $2.5068$ for chemtrails--Gates. Dual-discrete scores were
$4.2537$ versus $2.6880$ for SCoPEx and $3.7303$ versus $1.5297$ for
chemtrails--Gates.  Structural comparison was less favourable.  Breadth fell within
...
\paragraph{Interpretation.}
D-net holds topology fixed while persona labels change, which isolates a
composition contrast. It does not provide
...
\paragraph{Implication.}
The qualitative composition pattern recurs in
...
\paragraph{Interpretation.}
The two orderings address different comparisons because the H condition has different compositions in the generated-topology and D-net analyses.  The label H
...
\section{Relation to the prior arXiv preprint and adjacent literature}
\label{sec:discussion-literature}

\subsection{The prior auditor--node study as prior method, not a benchmark result}
...
heterogeneous branch--domain pairs.  Thus, the prior preprint itself does not
...
not a failed replication of a universal prior-study H$>$He hypothesis, because no
...
numerical reproduction of the prior preprint.  A direct conceptual replication
...
drop---rather than binary platform actions.  The constructed D-net hashtag graph had
...
\paragraph{Implication.}
In this thesis, Pfeffer's framework serves primarily to identify what the
...
These limitations restrict the claim to the documented prompts, graph
protocols, and auditor scores.  Under those conditions, auditor scores vary
with persona composition, topology, and scoring mode.  The campaign does not
establish human susceptibility,
...
composition interpretation recurs after a change in graph size and source under
