% Draft chapter fragment: validity, ethics, and future work.
% This file is intentionally candid about the Phase 2 harvest and must not be
% read as a claim of empirical Twitter/X validation.

\chapter{Validity, Ethics, and Reproducibility}
\label{chap:limitations}

The Phase~2 results are evidence about the behaviour of one configured
simulation campaign, not estimates of population effects and not a validated
digital twin of online climate discourse.  The harvest itself records
\texttt{thesisGrade=false}.  Its main value is therefore diagnostic: it shows
which patterns appeared under the specified graph generators, personas,
articles, language model, and scoring instruments, while exposing where those
patterns are sensitive to design choices.

\subsection{Internal and statistical validity}

\paragraph{One replicate.}
Every grid cell was run once with \texttt{graphRandomSeed=42} ($N=1$).
Events from the same run are dependent because they share a graph, node
states, inboxes, trust updates, and article history.  Article-cells and scored
events consequently cannot be treated as independent experimental
replicates.  The reported means, rank orders, and exploratory tests are
...
CIKM chain length of 30.  This horizon is neither an empirical firestorm
...
that appears irreversible at tick eight may recover at tick nine.
...
The same model family generates the messages and judges them against five
author-curated questions.  Shared model priors can create correlated generator
...
\texttt{nScored}$\leq 1$ despite real LLM calls.  They are kept in the ledger
...
observed retweets, replies, or follower ties.  Hydration of the reported
814,924 tweet identifiers did not occur: the accessible OSF identifiers were
stored in lossy scientific notation, the Mendeley identifier dump was not
retrieved, and no Twitter/X bearer token was available.  The fallback therefore
...
firestorms.  The failed DTFS score (\(0.2754<0.70\)) is evidence against the
configured validation threshold, not a near-success to be rescued by
individual metric matches.  Breadth similarity alone does not establish a
digital twin.
...
\subsection{Reproducibility and resource constraints}
...
\subsection{Privacy, research ethics, and dual use}

\paragraph{API privacy and cost.}
Sending content to hosted APIs transfers data to a third party and may create
retention, jurisdiction, and terms-of-service obligations.  Although this
campaign uses curated articles and synthetic personas, future hydration could
introduce usernames, tweet text, profile fields, or deleted content.  Public
...
human evaluation is a validity limitation and an ethical safeguard: incomplete
templates must not be presented as participant data.
...
\subsection{Priority future work}

The most consequential next step is not a larger unreplicated grid.  It is a
smaller, preregistered validation study with independent seeds, matched
