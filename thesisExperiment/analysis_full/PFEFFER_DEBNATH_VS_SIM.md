...
H and He share **identical** edge sets. Clustering coefficients therefore match the reconstructed structure. Homophily changes with labels. Newman--Girvan modularity also depends on a specified partition, but the homogeneous one-community partition is necessarily **Q=0**.
...
| **Clustering** (Outlook **#3** network clusters / transitivity) | n=63, 228 stored arcs / **171 unique undirected pairs**. Mean unique degree **5.43**; chemtrails hub unique neighbours **33**. Global transitivity **0.315**; mean local clustering **0.515**. On unique pairs, six assigned graph clusters have Q **0.5489**; four assigned discourse identities have Q **0.5322**; the binary conspiracy cut has Q **0.4297**. | **Same topology** as the reconstructed stand-in. Mixed BP-family labels have Q **0.4908**; the mixed conspiracy cut has Q **0.4297**. The homogeneous one-community partition has Q **0**, not 0.457. | Pre-wired same-cluster closure is strong enough for Pfeffer’s “echo from several neighbours” *structurally*. Modularity describes a named label partition; clustering describes edge closure and does not change when labels change. | Not Watts–Strogatz generative clustering (this is a documented co-occurrence construction). Not retweet triadic closure. Not community detection: the six graph clusters and discourse/persona labels are assigned partitions. Not Debnath eigenvector centrality re-estimated. |
...
1. **Clustering is a reconstructed-structure result, not a sim result.** Transitivity 0.315 and local clustering 0.515 live on the hashtag stand-in *before* any LLM call. D-net copies that graph; it does not grow clusters. Modularity is partition-dependent: graph clusters Q=0.5489, assigned identities Q=0.5322, mixed BP families Q=0.4908, binary conspiracy cut Q=0.4297, and a homogeneous one-community partition Q=0.
