 ...
 *
 * Polls /workspace/.env, thesisExperiment/.env, and KEY_READY.md every 20s up to
 * ~8 min if the key is missing. Probes one continuous cell until LLM usage > 0,
...
Polled \`/workspace/.env\`, \`thesisExperiment/.env\`, and \`KEY_READY.md\` every 20s for up to ~8 minutes (${polls.length} checks). Still missing or placeholder.
...
async function main() {
  ...
  if (!polled.ok) {
    ...
    campaign.abortReason =
      "OPENAI_API_KEY missing/placeholder after ~8 min poll of /workspace/.env, thesisExperiment/.env, and KEY_READY.md. Refusing to invent MI/MPR.";
